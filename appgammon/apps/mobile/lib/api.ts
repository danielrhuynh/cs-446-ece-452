...
export type StartSeriesRes = InferResponseType<typeof client.games[":id"]["series"]["start"]["$post"], 200>;
...
export async function proposeDouble(sessionId: string, gameId: string) {
  ...
  const res = await apiFetch(
    `/games/${id}/double?action=propose&game_id=${gameId}`,
  );
  return unwrap<{ ok: true }>(res);
}
...
export async function respondToDouble(
  ...
  const id = normalizeId(sessionId);
  const res = await apiFetch(
    `/games/${id}/double?action=${action}&game_id=${gameId}`,
  );
  return unwrap<{ ok: true }>(res);
}
