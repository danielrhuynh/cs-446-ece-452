...
import { subscribeGame } from "../utils/game-events";
...
export const gameRoutes = new Hono()
  .post(
    ...
    describeRoute({
      ...
      description: "Host-only. Creates a best-of-N series with the first game and opening dice roll.",
      requestBody: {
        required: true,
        content: { "application/json": { schema: jsonSchema(startSeriesPayloadSchema) } },
      },
      responses: {
        200: { description: "Series started", content: { "application/json": { schema: jsonSchema(seriesStateSchema) } } },
        ...
        400: { description: "Failed to start", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
      },
    }),
  ...
  .get(
    ...
    describeRoute({
      ...
      responses: {
        200: { description: "Current state", content: { "application/json": { schema: jsonSchema(seriesStateSchema) } } },
        401: { description: "Unauthorized", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
        404: { description: "No active series", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
      },
    }),
    async (c) => {
    const sessionId = normalizeSessionId(c.req.param("id"));
  ...
  .put(
    ...
    describeRoute({
      ...
      description: "Submit a sequence of moves for the current turn. Uses optimistic locking via version.",
      requestBody: {
        required: true,
        content: { "application/json": { schema: jsonSchema(submitMovesPayloadSchema) } },
      },
      responses: {
        200: { description: "Moves accepted", content: { "application/json": { schema: jsonSchema(okResponse) } } },
        400: { description: "Invalid moves or version mismatch", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
        401: { description: "Unauthorized", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
      },
    }),
    ...
    async (c) => {
      ...
      const result = await gameService.submitMoves(
        game_id,
        claims.sub,
        version,
        moves,
  ...
  .post(
    ...
    describeRoute({
      ...
      description: "Propose, accept, or decline a double. Query params: ?action=propose|accept|decline&game_id=UUID",
      responses: {
        200: { description: "Action successful", content: { "application/json": { schema: jsonSchema(okResponse) } } },
        400: { description: "Invalid action or state", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
        401: { description: "Unauthorized", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
      },
    }),
    async (c) => {
  ...
  .post(
    ...
    describeRoute({
      ...
      responses: {
        200: { description: "Dice rolled", content: { "application/json": { schema: jsonSchema(okResponse) } } },
        400: { description: "Cannot roll in this phase", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
        401: { description: "Unauthorized", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
      },
    }),
    async (c) => {
  ...
  .post(
    ...
    describeRoute({
      ...
      description: "Send an emote to the opponent. Rate limited to 1 per 3 seconds. Query param: ?id=emoteId",
      responses: {
        200: { description: "Emote sent", content: { "application/json": { schema: jsonSchema(okResponse) } } },
        400: { description: "Invalid emote", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
        429: { description: "Rate limited", content: { "application/json": { schema: jsonSchema(errorResponse) } } },
      },
    }),
    async (c) => {
    const sessionId = normalizeSessionId(c.req.param("id"));
...
export const gameSSEController = new Hono().get(
  "/:id/events",
  async (c) => {
    const sessionId = normalizeSessionId(c.req.param("id"));
    const claims = await authenticateRequest(c, sessionId);
    ...
    return streamSSE(c, async (stream) => {
      ...
      const unsubscribe = subscribeGame(sessionId, (event) => {
