...
import type { Board, Bar, BorneOff, Dice, DiceUsed, Move, PlayerRole } from "./game-types";
import { getAvailableDice, initializeDiceUsed, markDieUsed } from "./dice";
import { applyMove } from "./board";
import { getValidMoves, getDieValueForMove } from "./move-validation";
...
export function getMaxDiceUsable(
  ...
  for (let i = 0; i < diceUsed.length; i++) {
    ...
    for (const move of validMoves) {
      ...
      const subMax = 1 + getMaxDiceUsable(
        result.board,
        result.bar,
        result.borneOff,
        dice,
